...
- 日志内容中的时间戳使用运行环境的本地时区解析
- 输出使用 ISO 8601 格式带时区偏移（如 `2026-04-12T15:04:05+08:00`）

> **重要**：ETL 的时区必须与 mementomori-helper 保持一致，否则时间戳解析会产生偏差。
>
> mementomori-helper 使用 `DateTime.Now` 输出日志时间，ETL 使用 `time.Local` 解析。
> 两者时区一致才能保证时间正确。Docker 部署时请在两个容器中设置相同的 `TZ` 环境变量（如 `TZ=Asia/Shanghai`）。
...
- 时间戳使用运行环境本地时区，Docker 部署建议设置 `TZ` 环境变量
- 输出目录不存在时会自动创建
